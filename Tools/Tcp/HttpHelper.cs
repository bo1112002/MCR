...
namespace Tools.Tcp
{
    ...
    public class HttpHelper
    {
        ...
        /// <summary>get请求</summary>
        public static string HttpGet(string Url, Dictionary<string, string> dicPost = null )
        {
            StringBuilder postDataStr = new StringBuilder("T=" + DateTime.Now.ToFileTime() );
            if (dicPost != null)
            {
                foreach (string key in dicPost.Keys)
                {
                    postDataStr.Append('&');
                    postDataStr.Append(string.Format("{0}={1}", key, dicPost[key]));
                }
            }
            ...
            if( Url.Contains('?') )
            {
                getURL = Url + "&" + postDataStr.ToString() ;
            }
            else
            {
                getURL = Url + "?" + postDataStr.ToString() ;
